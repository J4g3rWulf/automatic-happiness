...
de coleta do RecycleApp (espelho exato de RecyclingPointsData.ALL).
...
Comportamento:
    - Idempotente: usa set() sem merge, logo reexecutar sobrescreve sem duplicar
    - Usa batch writes (máx. 500 por lote) para eficiência
...
# IDs a remover — adicione os IDs desejados e esvazie após rodar
to_delete = []

# ── Dados (espelho exato de RecyclingPointsData.ALL) ──────────────────────────
...
points = [
    ...
    },
    {
        "id": "pev_dende",
        "name": "PEV Dendê",
        "address": "Av. Paranapuã, 2112 - Tauá, Rio de Janeiro - RJ, 21910-005",
        "latitude": -22.800864054082407,
        "longitude": -43.182710110735414,
        "materials": ["Papel", "Plástico", "Vidro", "Metal"],
        "type":"PEV",
    },
    {
        ...
        "address": "R. Eulino Nogueira, 1 - Campo Grande, Rio de Janeiro - RJ, 23036-320",
        "latitude": -22.934378918102833,
        "longitude": -43.57541760189042,
        "materials": ["Papel", "Plástico", "Vidro", "Metal"],
        "type":"PEV",
    },
    {
        "id": "pev_marechal_hermes",
        "name": "PEV Marechal Hermes",
        "address": "R. Gen. Savaget, 326 - Marechal Hermes, Rio de Janeiro - RJ, 21610-260",
        "latitude": -22.864111322807478,
        "longitude": -43.373241789562584,
        "materials": ["Papel", "Plástico", "Vidro", "Metal"],
        "type":"PEV",
    },

    # ── Ecopontos Comlurb — Zona Central ─────────────────────────────────────
    {
        "id": "ecoponto_arara",
        "name": "Ecoponto Arará",
        ...
        "materials": ["Lixo domiciliar", "Entulho", "Bens inservíveis", "Galhadas"],
        "type": "ECOPONTO",
    },
    {
    ...
    # ── Ecopontos Comlurb — Jacarepaguá / Barra / Recreio ────────────────────
    {
        "id": "ecoponto_areal",
        "name": "Ecoponto Areal",
    ...
    # ── Ecopontos Comlurb — Zona Sul / Rocinha / Vidigal ─────────────────────
    {
        "id": "ecoponto_199",
        "name": "Ecoponto 199",
    ...
    # ── Ecopontos Light Recicla ───────────────────────────────────────────────
    {
        ...
        "materials": ["Plástico", "Metal", "Vidro", "Papel", "Óleo vegetal"],
        "type": "ECOPONTO_LIGHT",
    },
    {
...
# ── Upload em lotes (batch writes, máx. 500 por lote) ────────────────────────

def upload_in_batches(points: list, batch_size: int = 400):
    total   = len(points)
    uploaded = 0
    ...
    for i in range(0, total, batch_size):
        ...
        for point in chunk:
            doc_ref = db.collection(COLLECTION).document(point["id"])
            # Exclui "id" do payload — ele já é o document ID
            payload = {k: v for k, v in point.items() if k != "id"}
            batch.set(doc_ref, payload)

        batch.commit()
        uploaded += len(chunk)
        print(f"  ✓ {uploaded}/{total} pontos enviados")

    return uploaded

def delete_points(ids: list):
    for doc_id in ids:
...
def update_metadata_timestamp():
    ...
    db.collection("metadata").document("recycling_points").update({
        "last_updated": timestamp
    })
    print(f"  ✓ last_updated atualizado para {timestamp}")

# ── Main ──────────────────────────────────────────────────────────────────────

if __name__ == "__main__":
    print(f"Iniciando upload de {len(points)} pontos para '{COLLECTION}'...")
    total = upload_in_batches(points)
    if to_delete:
        print(f"Deletando {len(to_delete)} ponto(s)...")
        delete_points(to_delete)
    update_metadata_timestamp()
    print(f"\n✅ Concluído! {total} pontos cadastrados no Firestore.")
